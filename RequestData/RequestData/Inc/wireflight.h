...
struct InterfaceF35Data{
	...
	bool dCommunicationCom1;        //A旋钮
	bool dCommunicationCom2;         //B旋钮
	bool dNavigationRadio;           //C/BUR旋钮
...
{
	double dLandingLightSwitch;            //   LIGHT LANDING                 变量    bool
	double dLandingGear;                   //	GEAR HANDLE POSITION          变量    下位置是 1 收上 是 0   
	double dPakingBreak;                  //    BRAKE PARKING POSITION        变量     
	double dAVionMasterSwitch;               // AVIONICS MASTER SWITCH    旋钮    bool
	double dTaiHookHandle;                   // TAILHOOK HANDLE     true 放拦阻钩
...
struct sInAH1ZData
{
	int HelicpterAcceRod;        // 直升机总距杆 杆位移  
	int IcsKnobButton;           //ICS旋钮
